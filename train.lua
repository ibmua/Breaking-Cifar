...
-- local criterion = cast
-- 					(
-- 					nn.ClassSimplexCriterion(opt.num_classes)
-- 					)

-- local criterion = cast(nn.ClassNLLCriterion())
...
engine.hooks.onStartEpoch = function(state)
   ...
   if torch.type(opt.epoch_step) == 'number' and epoch % opt.epoch_step == 0 or
	  ...
	  opt.learningRate = math.max(0.0007, opt.learningRate)
	  ---
