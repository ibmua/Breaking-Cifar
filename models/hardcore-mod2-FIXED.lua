...
local function createModel(opt)
	...
	local function wide_basic(nInputPlane, nOutputPlane, stride	,	groups)
		...
		for i,v in ipairs(conv_params) do
			if i == 1 then
				...
				convs:add	(
							Convolution(nInputPlane,nBottleneckPlane, table.unpack(v)		, groups)
							)
			else
				...
				convs:add	(
							Convolution(nBottleneckPlane,nBottleneckPlane, table.unpack(v)	, groups)
							)
			end
		end
	...
	local function layer(block, nInputPlane, nOutputPlane, count, stride , groups)
		...
		for i=2,count do
			s:add	(
					block(nOutputPlane, nOutputPlane, 1 		,0	, groups)
					)
		end
		return s
	end
