...
local function createModel(opt)
	...
	local function layer(block, nInputPlane, nOutputPlane, count, stride , groups)
		...
		s:add(block(nInputPlane, nOutputPlane, stride 			, 1))
		for i=2,count do
	...
	do
		...
		local nStages = torch.Tensor{48, 16*k, 32*k, 64*k}
		...
		model:add(layer(wide_basic, nStages[2], nStages[3], n, 2		,64)) -- Stage 2 (spatial size: 16x16)
		model:add(layer(wide_basic, nStages[3], nStages[4], n, 2		,128)) -- Stage 3 (spatial size: 8x8)
