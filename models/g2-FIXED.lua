...
local function createModel(opt)
	...
	local function wide_basic(nInputPlane, nOutputPlane, stride	,	groups)
		...
		for i,v in ipairs(conv_params) do
			if i == 1 then
				...
				-- print	(	nInputPlane,nBottleneckPlane, table.unpack(v)		, 8)
				print	(	nInputPlane,nBottleneckPlane,		3,3,	stride,stride,	1,1			, groups )

				convs:add	(
							Convolution(nInputPlane,nBottleneckPlane,		3,3,	stride,stride,	1,1			, groups)
							)
			else


				print	(	nBottleneckPlane,nBottleneckPlane,	3,3,	1,1,			1,1			, groups)
				...
				convs:add	(
							Convolution(nBottleneckPlane,nBottleneckPlane,	3,3,	1,1,			1,1			, groups)
							)
			end
		end
	...
	do
		...
		model:add(layer(wide_basic, nStages[2], nStages[3], n, 2		,8	)) -- Stage 2 (spatial size: 16x16)
		model:add(layer(wide_basic, nStages[3], nStages[4], n, 2		,8	)) -- Stage 3 (spatial size: 8x8)
