...
Gs	=	6 -- 40-depth -- 6x residual blocks | 2-width -> 3x residual blocks | 2xWidth x 2xGroups
...
local function createModel(opt)
	...
	local function wide_basic(nInputPlane, nOutputPlane, stride	,	groups , table)
		local nBottleneckPlane = nOutputPlane

		local conv_params =
			{
				{nInputPlane				,	nBottleneckPlane			,	1,1	,	1,1,			0,0	,	1	},
				-- {nBottleneckPlane			,	nBottleneckPlane	*	Gs	,	3,3	,	stride,stride,	1,1	,	nBottleneckPlane	},
				{nBottleneckPlane			,	nBottleneckPlane	*	Gs	,	3,3	,	stride,stride,	1,1	,	nBottleneckPlane/4	},
				-- {nBottleneckPlane	*	Gs	,	nOutputPlane		*	Gs	,	3,3	,	1,1,			1,1	,	Gs	},
				{nBottleneckPlane	*	Gs	,	nOutputPlane		*	Gs	,	1,1	,	1,1,			0,0	,	Gs	},
	...
	local function layer(block, nInputPlane, nOutputPlane, count, stride 	, groups)
		...
		s:add	(
						block(nInputPlane, nOutputPlane, stride 			, groups, nn.CAddTable())
				)
		for i=2,count do
			s:add	(
						block(nOutputPlane, nOutputPlane, 1 				, groups, nn.CMulTable())
					)
		end
		return s
	end
	...
	do
		...
		model
			:add(Convolution(3,nStages[1],	3,3,	1,1,	1,1			,1	)) -- one conv at the beginning (spatial size: 32x32)
		...
		-- model:add(layer(wide_basic, nStages[3], nStages[4], n, 2		,nStages[3]	)) -- Stage 3 (spatial size: 8x8)
			...
			:add(layer(wide_basic, nStages[1], nStages[2], n, 1		,1	)) -- Stage 1 (spatial size: 32x32)
			:add(layer(wide_basic, nStages[2], nStages[3], n, 2		,1	)) -- Stage 2 (spatial size: 16x16)
