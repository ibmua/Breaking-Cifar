...
local function createModel(opt)
	...
	local function wide_basic(nInputPlane, nOutputPlane, stride	,	groups)
		...
		for i,v in ipairs(conv_params) do
			if i == 1 then
				...
				convs:add(Convolution(nInputPlane,nBottleneckPlane,table.unpack(v))			, groups)
			else
				convs:add(SBatchNorm(nBottleneckPlane)):add(ReLU(true))
				if opt.dropout > 0 then
					convs:add(Dropout())
				end
				convs:add(Convolution(nBottleneckPlane,nBottleneckPlane,table.unpack(v))	, groups)
			end
		end

		local shortcut = nInputPlane == nOutputPlane and
			nn.Identity() or
			Convolution(nInputPlane,nOutputPlane,1,1,stride,stride,0,0						, groups)
	...
	local function layer(block, nInputPlane, nOutputPlane, count, stride , groups)
		...
		s:add(block(nInputPlane, nOutputPlane, stride 			, groups))
		for i=2,count do
			s:add(block(nOutputPlane, nOutputPlane, 1 		,0	, groups))
		end
		return s
	end
	...
	do
		...
		local nStages = torch.Tensor{48, 12*k, 12*2*k, 12*2*2*k}

		model:add(Convolution(3,nStages[1],3,3,1,1,1,1					,3)) -- one conv at the beginning (spatial size: 32x32)
		model:add(layer(wide_basic, nStages[1], nStages[2], n, 1		,16)) -- Stage 1 (spatial size: 32x32)
		model:add(layer(wide_basic, nStages[2], nStages[3], n, 2		,32*k)) -- Stage 2 (spatial size: 16x16)
		model:add(layer(wide_basic, nStages[3], nStages[4], n, 2		,64*k)) -- Stage 3 (spatial size: 8x8)
