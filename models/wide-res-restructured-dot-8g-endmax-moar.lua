...
local function createModel(opt)
		...
		local conv_params =
			{
				...
				{nBottleneckPlane			,	nBottleneckPlane*2	*	Gs	,	3,3	,	1,1,			1,1	,	Gs		*	groups	},
				{nBottleneckPlane*2	*	Gs	,	nOutputPlane		*	Gs	,	1,1	,	stride,stride,	0,0	,	Gs					},
	...
	do
		...
		-- model:add(layer(wide_basic, nStages[3], nStages[4], n, 2		,nStages[3]	)) -- Stage 3 (spatial size: 8x8)
			...
			:add(layer(wide_basic, nStages[1], nStages[2], n, 1		,4	)) -- Stage 1 (spatial size: 32x32)

			:add(layer(wide_basic, nStages[2], nStages[3], n, 2		,2	)) -- Stage 2  (spatial size: 16x16)
			-- :add(layer(wide_basic, nStages[3], nStages[3], n, 1		,2	)) -- Stage 2.5(spatial size: 16x16)

			:add(layer(wide_basic, nStages[3], nStages[4], n, 2		,1	)) -- Stage 3 (spatial size: 8x8)
			-- :add(layer(wide_basic, nStages[4], nStages[4], n, 1		,1	)) -- Stage 2.5(spatial size: 16x16)
