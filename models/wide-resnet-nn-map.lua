...
-- local Convolution = nn.SpatialConvolutionMap
local function Convolution(i,o, ...)
	local arg = {...}
	-- model:add()
	...
	-- local tables = nn.tables.oneToOne( arg[1] )
	print	(
				tables
			,	unpack(arg)
			)
	return nn.SpatialConvolutionMap( tables , unpack(arg) )
		-- :noBias()
end
