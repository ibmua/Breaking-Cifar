...
local function createModel(opt)
	...
	local function wide_basic(nInputPlane, nOutputPlane, stride	,	groups)
		...
		for i,v in ipairs(conv_params) do
			if i == 1 then
				...
				convs:add(Convolution(nInputPlane,
													nBottleneckPlane
													,table.unpack(v))			, math.min(nInputPlane , nBottleneckPlane))
			else
				...
				convs:add(Convolution(
						nBottleneckPlane
						,nOutputPlane,table.unpack(v))						, nOutputPlane)
			end
		end

		local shortcut = nInputPlane == nOutputPlane and
			nn.Identity() or
			Convolution(nInputPlane,nOutputPlane,1,1,stride,stride,0,0			, math.min(nInputPlane , nOutputPlane))
	...
	do
		...
		-- local nStages = torch.Tensor{128, 16*k, 32*k, 64*k}
		local nStages = torch.Tensor{128, 16*k, 32*k, 64*k}
		...
		-- THIS SHOULD PROBABLY SUCK model:add(Convolution(nStages[4] , opt.num_classes , 1,1 , 1,1 , 0,0		,1))
		-- model:add(Convolution(nStages[4] , opt.num_classes ,3,3, 1,1, 1,1		,1))
